...
describe Account do
  ...
  context "#deposite" do
    it "should stored the date and the number" do
      account.deposite(50, date1)
      expect( account.transaction ).to eq({ "01/01/2017" => [50] })
      account.deposite(30, date2)
      expect( account.transaction ).to eq({ "01/01/2017" => [50], "02/01/2017" => [30] })
    end
    it "should stored just the number when the date is already exist" do
      account.deposite(50, date1)
      account.deposite(30, date1)
      expect( account.transaction ).to eq({ "01/01/2017" => [50,30] })
    end
  end

  context "#withdraw" do
    it "should stored the date and the minus number" do
      account.deposite(100, date1)
      account.withdraw(30, date1)
      expect( account.transaction ).to eq({ "01/01/2017" => [100,-30] })
      account.withdraw(20, date2)
      expect( account.transaction ).to eq({ "01/01/2017" => [100,-30], "02/01/2017" => [-20] })
    end
  ...
  context "#print_bank_statement" do
    ...
    it "should print transaction" do
      account.deposite(100, date1)
      account.withdraw(20, date2)
      expect( account.transaction ).to eq({ "01/01/2017" => [100], "02/01/2017" => [-20] })
